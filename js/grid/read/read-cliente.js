...
function atualizarGrid(resultados) {
    ...
    if (resultados.length > 0) {
        resultados.forEach(item => {
            ...
            // Coluna Tipo (Física ou Jurídica)
            const tipoColuna = document.createElement('td');
            tipoColuna.textContent = item.cpf ? 'Física' : item.cnpj ? 'Jurídica' : 'Tipo desconhecido';
            row.appendChild(tipoColuna);

            // Coluna Documento (CPF ou CNPJ)
            const documentoColuna = document.createElement('td');
            documentoColuna.textContent = item.cpf ? `CPF: ${item.cpf}` : item.cnpj ? `CNPJ: ${item.cnpj}` : 'Documento desconhecido';
            row.appendChild(documentoColuna);   
...
) 
{
    ...
    if (acao === 'update') {
        ...
        mensagemModalAcao.textContent = `Editando o cliente: ${nome || ''}`;

        // Conteúdo dinâmico para o formulário de edição
        modalContent.innerHTML = `
            <form id="updateForm" action="../config/crud-cliente/update.php" method="POST" enctype="multipart/form-data">
                ...
                <input type="text" id="updateNome" name="nome" class="input" class="form-control" value="${nome || ''}">
                </div>
                
                <div class="input-group">
                <label for="updateEmail">Email</label>
                <input type="email" id="updateEmail" name="email" class="input" class="form-control" value="${email || ''}">
                </div>

                <div class="input-group">
                <label for="updateTelefone">Telefone</label>
                <input type="text" id="updateTelefone" name="telefone" class="input" class="form-control" value="${telefone || ''}">
                </div>
                ...
                ${cpf ? `
                    ...
                    <input type="text" id="updateCPF" name="cpf" class="input" class="form-control" value="${cpf || ''}">
                    </div>
                  
                    <div class="input-group">
                    <label for="updateRG">RG</label>
                    <input type="text" id="updateRG" name="rg" class="input" class="form-control" value="${rg || ''}">
                    </div>
                ...
                ${cnpj ? `
                    ...
                    <input type="text" id="updateCNPJ" name="cnpj" class="input" class="form-control" value="${cnpj || ''}">
                    </div>

                    <div class="input-group">
                    <label for="updateIE">Inscrição Estadual (IE)</label>
                    <input type="text" id="updateIE" name="ie" class="input" class="form-control" value="${ie || ''}">
                    </div>

                    <div class="input-group">
                    <label for="updateRazaoSocial">Razão Social</label>
                    <input type="text" id="updateRazaoSocial" name="razao" class="input" class="form-control" value="${razao_social || ''}">
                    </div>
                    
                    <div class="input-group">
                    <label for="updateNomeFantasia">Nome Fantasia</label>
                    <input type="text" id="updateNomeFantasia" name="fantasia" class="input" class="form-control" value="${nome_fantasia || ''}">
                    </div>
                ` : ''}

                <div class="input-group">
                <label for="updateRua">Rua</label>
                <input type="text" id="updateRua" name="rua" class="input" class="form-control" value="${rua || ''}">
                </div>

                <div class="input-group">
                <label for="updateNumeroCasa">Número da Casa</label>
                <input type="text" id="updateNumeroCasa" name="numero" class="input" class="form-control" value="${numero_casa || ''}">
                </div>

                <div class="input-group">
                <label for="updateBairro">Bairro</label>
                <input type="text" id="updateBairro" name="bairro" class="input" class="form-control" value="${bairro || ''}">
                </div>
                    
                <div class="input-group">
                <label for="updateCidade">Cidade</label>
                <input type="text" id="updateCidade" name="cidade" class="input" class="form-control" value="${cidade || ''}">
                </div>

                <div class="input-group">
                <label for="updateEstado">Estado</label>
                <input type="text" id="updateEstado" name="estado" class="input" class="form-control" value="${estado || ''}">
                </div>    

                <div class="input-group">
                <label for="updateUf">UF</label>
                <input type="text" id="updateUf" name="uf" class="input" class="form-control" value="${uf || ''}">
                </div>

                <div class="input-group">
                <label for="updateCEP">CEP</label>
                <input type="text" id="updateCEP" name="cep" class="input" class="form-control" value="${cep || ''}">
                </div>

                <div class="input-group">
                <label for="updateComplemento">Complemento</label>
                <input type="text" id="updateComplemento" name="complemento" class="input" class="form-control" value="${complemento || ''}">
                </div>

                <div class="input-group">
                <label for="updatePontoReferencia">Ponto de Referência</label>
                <input type="text" id="updatePontoReferencia" name="referencia" class="input" class="form-control" value="${ponto_referencia || ''}">
                </div>
        ...
        modalContent.innerHTML = `
        <form id="deleteForm" action="../config/crud-cliente/delete.php" method="POST">
