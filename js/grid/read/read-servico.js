function carregarItens() {
    ...
    xhr.open('GET', '../config/crud-servico/read.php', true); // URL para carregar dados dos clientes
    xhr.onload = function () {
        if (xhr.status === 200) {
...
) 
{
    ...
    if (acao === 'update') {
        ...
        mensagemModalAcao.textContent = `Editando o serviço: ${nome_servico_produto || ''}`;

        // Conteúdo dinâmico para o formulário de edição
        ///////////////////////AQUI VOCE PODE MEXER////////////////////
        modalContent.innerHTML = `
            <form id="updateForm" action="../config/crud-servico/update.php" method="POST" enctype="multipart/form-data">
    
            <!-- Campo oculto para ID do serviço -->
            <input type="hidden" id="updateIdServicoProduto" name="idServico" value="${id_servico_produto || ''}">
            ...
            <div class="mb-3">
                <label for="updateDescricao" class="form-label">Descrição do Serviço</label>
        ...
        mensagemModalAcao.textContent = `Tem certeza que deseja excluir o serviço: ${nome_servico_produto || ''}?`;

        // Conteúdo dinâmico para o formulário de exclusão
        modalContent.innerHTML = `
        <form id="deleteForm" action="../config/crud-servico/delete.php" method="POST">
            <!-- Campo oculto para o ID -->
            <input type="hidden" id="deleteId" name="idServico" value="${id_servico_produto || ''}">
